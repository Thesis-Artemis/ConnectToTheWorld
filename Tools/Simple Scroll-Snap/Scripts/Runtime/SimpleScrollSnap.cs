...
namespace DanielLochner.Assets.SimpleScrollSnap
{
    ...
    public class SimpleScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        public bool scrollCustomize;
        public int limMinPanel;
        public int limMaxPanel;
        ...
        public void SetSrollCustomize(int _limMinPanel, int _limMaxPanel){
            if(infinitelyScroll && NumberOfPanels <= 0){
                return;
            }
            scrollCustomize = true;
            limMinPanel = _limMinPanel;
            limMaxPanel = _limMaxPanel;
        }
        #if UNITY_EDITOR
        ...
        {
            // Debug.LogError(panelNumber + " - " + CurrentPanel + " - " + TargetPanel);
            if(scrollCustomize){
                if(panelNumber < CurrentPanel && panelNumber <= limMinPanel){
                    panelNumber = limMinPanel;
                }else if(panelNumber > CurrentPanel && panelNumber >= limMaxPanel){
                    panelNumber = limMaxPanel;
                }
            }
        ...
        public void GoToPreviousPanel()
        {
            ...
            // if(scrollCustomize && NearestPanel <= limMinPanel){
            //     return;
            // }
            if (NearestPanel != 0)
            {
        ...
        {
            NearestPanel = DetermineNearestPanel();
            // if(scrollCustomize && NearestPanel >= limMaxPanel){
            //     return;
            // }
            if(NearestPanel == NumberOfPanels - 2){
                return;
            }
        ...
        public void AddFromExistObject(GameObject panel, int index)
        {
            if (NumberOfPanels != 0 && (index < 0 || index > NumberOfPanels))
            {
                Debug.LogError("<b>[SimpleScrollSnap]</b> Index must be an integer from 0 to " + NumberOfPanels + ".", gameObject);

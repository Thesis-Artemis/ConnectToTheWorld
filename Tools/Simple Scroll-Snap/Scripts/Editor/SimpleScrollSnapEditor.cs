...
namespace DanielLochner.Assets.SimpleScrollSnap
{
    ...
    public class SimpleScrollSnapEditor : Editor
    {
        ...
        private SerializedProperty scrollCustomize, limMinPanel, limMaxPanel;
        ...
        private void OnEnable()
        {
            ...
            scrollCustomize = serializedObject.FindProperty("scrollCustomize");
            limMinPanel = serializedObject.FindProperty("limMinPanel");
            limMaxPanel = serializedObject.FindProperty("limMaxPanel");
        }
        ...
        private void ScrollCustomize(){
            if(simpleScrollSnap.infinitelyScroll || simpleScrollSnap.NumberOfPanels == 0){
                simpleScrollSnap.scrollCustomize = false;
                simpleScrollSnap.limMinPanel = 0;
                simpleScrollSnap.limMaxPanel = 0;
                return;
            }
            if(!simpleScrollSnap.scrollCustomize){
                simpleScrollSnap.limMinPanel = 0;
                simpleScrollSnap.limMaxPanel = 0;
            }
            if (!simpleScrollSnap.infinitelyScroll && simpleScrollSnap.NumberOfPanels > 0){
                EditorGUILayout.PropertyField(scrollCustomize, new GUIContent("Scroll Customize"));
                if(simpleScrollSnap.scrollCustomize){
                    EditorGUI.indentLevel++;
                    EditorGUILayout.IntSlider(limMinPanel, 0, simpleScrollSnap.NumberOfPanels - 1, new GUIContent("Lim Min Panel"));
                    EditorGUILayout.IntSlider(limMaxPanel, 0, simpleScrollSnap.NumberOfPanels - 1, new GUIContent("Lim Max Panel"));
                    EditorGUI.indentLevel--;
                };
            }
        }
